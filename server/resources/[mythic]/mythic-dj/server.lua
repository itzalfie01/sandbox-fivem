...
function RegisterCallbacks()
	Callbacks:RegisterServerCallback("mythic-dj:server:songInfo", function(source, data, cb)
		cb(previousSongs)
	end)

	Callbacks:RegisterServerCallback("mythic-dj:server:playMusic", function(source, data, cb)
		local src = source
	...
	Callbacks:RegisterServerCallback("mythic-dj:server:changeVolume", function(source, data, cb)
		local src = source
		local zoneNum = data.zoneNum
	...
	Callbacks:RegisterServerCallback("mythic-dj:server:stopMusic", function(source, data, cb)
		local src = source
		...
		TriggerClientEvent("mythic-dj:client:playMusic", src, { zone = data.zoneNum })
	end)

	Callbacks:RegisterServerCallback("mythic-dj:server:pauseMusic", function(source, data, cb)
		local src = source
		local zoneLabel = data.zoneName
		if Config.Locations[data.zoneNum].playing then
			Config.Locations[data.zoneNum].playing = nil
	...
	Callbacks:RegisterServerCallback("mythic-dj:server:resumeMusic", function(source, data, cb)
		local src = source
		local zoneLabel = data.zoneName
...
-- -- This is for that
-- RegisterNetEvent("mythic-dj:server:DestoryAll", function()
...
-- 	TriggerEvent("mythic-dj:server:DestoryAll")
-- end)
